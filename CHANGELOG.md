...
### 文件
- 更新 `docs/ARCHITECTURE-OVERVIEW.md` 至版本 2.0
  - 新增完整翻譯管線資料流：Whisper ASR（英文）→ ML Kit（簡體中文）→ opencc4j（台灣繁體中文）
  - 新增端到端資料流概覽圖
  - 新增翻譯管線摘要圖
  - 更新系統架構圖，加入 app 層的 Translation Layer
  - 更新時序圖，加入翻譯模型下載與即時翻譯流程
  - 更新模組職責，app 模組新增翻譯管線說明
  - 更新技術選型，新增 ML Kit Translate 17.0.3 與 opencc4j 1.14.0
  - 更新目錄結構，列出翻譯相關檔案
  - 更新 SDK 整合範例，包含翻譯使用方式
  - 新增第十一章：翻譯管線架構決策（含錯誤處理策略、設計理由、執行緒安全機制）
